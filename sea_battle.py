...
class OffBoardException(BoardException):
    def __str__(self):
        return "The shot was fired out of bounds!"


class OccupiedBoardException(BoardException):
    def __str__(self):
        return "These coordinates have already been shot at!"
    ...
    def shot(self, d):
        ...
        for ship in self.ships:
            if d in ship.dots:
                ...
                if len(ship.dots) == 3:
                    if not self.hide:
                        print("\033[31mThe three-deck flagship of your fleet is attacked..!\033[0m")
                        sleep(1.5)
                if ship.lives == 0:
                    self.count += 1
...
class AI(Player):
    def ask(self):
        ...
        print(f"Enemy's move:⚔ {d.y + 1} {d.x + 1}")
        return d
...
class User(Player):
    def ask(self):
        ...
        while True:
            ...
            if x not in x_index:
                print("'x' coordinate entered incorrectly!")
                continue
...
class Game:
    ...
    def before(self):
        ...
        sleep(1.5)
        print()
        print("\033[3mThe x coordinate is the letters a,b,c,d,e,f ")
        sleep(1.5)
        print("The y coordinate is the numbers 1,2,3,4,5,6")
        sleep(1.5)
        print("Entering coordinates - separated by a space!\033[0m")
        print()

    def print_boards_user(self):
        print("-" * 27)
        print("%23s " % "User board:")
        print(self.us.board)
        print("-" * 27)

    def print_boards_AI(self):
        print("%23s " % "Enemy board:")
        print(self.ai.board)
        print("-" * 27)
